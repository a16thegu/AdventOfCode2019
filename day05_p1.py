...
# In immediate mode, a parameter is interpreted as a value - if the parameter is 50, its 
# value is simply 50.

# Parameter modes are stored in the same value as the instruction's opcode. The opcode is 
# a two-digit number based only on the ones and tens digit of the value, that is, the 
# opcode is the rightmost two digits of the first value in an instruction. Parameter modes 
# are single digits, one per parameter, read right-to-left from the opcode: the first 
# parameter's mode is in the hundreds digit, the second parameter's mode is in the 
