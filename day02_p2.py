...
# memory to the program's values. A position in memory is called an address (for example, the first 
...
def computeIntcode(array) :
    index = 0
    while True:
        if array[index] == 1 :
            pos1 = array[index + 1]
            pos2 = array[index + 2]
            posSave = array[index + 3]
           ...
           print("Something went wrong at index: " + str(index) + " and array value: " + str(array[index]))
           break
...
for line in fh :
    input += [int(i) for i in line.split(',') if i.isdigit()] 

for noun in range(100) :
    for verb in range(100) :
        inputCopy = input.copy() # Lists cannot be copied with just inputCopy = input, it becomes a reference point (pointer) to the original list.
        inputCopy[1] = noun
        inputCopy[2] = verb
        output = computeIntcode(inputCopy)
